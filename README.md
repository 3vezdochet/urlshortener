...
Это писалось в песочнице без доступа к `proxy.golang.org`/`sum.golang.org`
(egress только на ограниченный список хостов) и без Docker — поэтому `pgx`,
`goose` и `testcontainers-go` не были ни загружены, ни собраны здесь. Сам код
написан сразу на них, без подмены на более лёгкие альтернативы. Локально, с
обычным доступом в интернет, донакати зависимости и досоздай `go.sum`:
